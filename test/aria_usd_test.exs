...
defmodule AriaUsdTest do
  ...
  describe "vrm_to_usd/3" do
    test "converts VRM to USD (mock when pxr unavailable)" do
      case AriaUsd.vrm_to_usd("/path/to/test.vrm", "/path/to/output.usd") do
        {:ok, status} ->
          assert is_binary(status)
    ...
    @tag :requires_pythonx
    test "returns error for non-existent VRM file" do
      case AriaUsd.ensure_pythonx() do
        :mock ->
          :ok

        :ok ->
          tmp_usd = tmp_usd_path("vrm_to_usd")

          try do
            assert {:error, _reason} = AriaUsd.vrm_to_usd("nonexistent.vrm", tmp_usd)
          after
            if File.exists?(tmp_usd), do: File.rm(tmp_usd)
          end
      end
    ...
    @tag :requires_pythonx
    test "converts VRM to USD with metadata preservation" do
      case AriaUsd.ensure_pythonx() do
        :mock ->
          :ok

        :ok ->
          # This test would require a VRM file, so we'll just test the function signature
          # Full integration test would be in dcc_mcp
          tmp_usd = tmp_usd_path("vrm_to_usd_metadata")

          try do
            # Test with metadata options
            vrm_extensions = %{"VRM" => %{"version" => "1.0"}}
            vrm_metadata = %{version: "1.0", title: "Test"}

            case AriaUsd.vrm_to_usd("test.vrm", tmp_usd,
                   vrm_extensions: vrm_extensions,
                   vrm_metadata: vrm_metadata
                 ) do
              {:error, _reason} ->
  ...
  describe "usd_to_tscn/2" do
    test "converts USD to TSCN (mock when pxr unavailable)" do
      case AriaUsd.usd_to_tscn("/path/to/test.usd", "/path/to/output.tscn") do
        {:ok, status} ->
          assert is_binary(status)
    ...
    @tag :requires_pythonx
    test "returns error for non-existent USD file" do
      case AriaUsd.ensure_pythonx() do
        :mock ->
          :ok

        :ok ->
          tmp_tscn = System.tmp_dir!() |> Path.join("test_#{:rand.uniform(1_000_000)}.tscn")

          try do
            assert {:error, _reason} = AriaUsd.usd_to_tscn("nonexistent.usd", tmp_tscn)
          after
            if File.exists?(tmp_tscn), do: File.rm(tmp_tscn)
          end
      end
    end
  end

  describe "tscn_to_usd/3" do
    test "returns error when tscn_data not provided" do
      assert {:error, _reason} = AriaUsd.tscn_to_usd("/path/to/test.tscn", "/path/to/output.usd")
    end

    @tag :requires_pythonx
    test "converts TSCN to USD with provided tscn_data" do
      case AriaUsd.ensure_pythonx() do
        :mock ->
          :ok
        ...
        :ok ->
          ...
          try do
            case AriaUsd.tscn_to_usd("test.tscn", tmp_usd, tscn_data: tscn_data) do
              {:ok, _message} ->
                # Verify USD file was created
                assert File.exists?(tmp_usd)

              {:error, _reason} ->
                # Might fail if path issues
                :ok
            end
          after
            if File.exists?(tmp_usd), do: File.rm(tmp_usd)
  ...
  describe "usd_to_unity_package/2" do
    test "converts USD to Unity package (mock when pxr unavailable)" do
      case AriaUsd.usd_to_unity_package("/path/to/test.usd", "/path/to/output.unitypackage") do
        {:ok, status} ->
          assert is_binary(status)
        ...
        :ok ->
          ...
          try do
            assert {:error, _reason} =
                     AriaUsd.usd_to_unity_package("nonexistent.usd", tmp_unitypackage)
          after
            if File.exists?(tmp_unitypackage), do: File.rm(tmp_unitypackage)
          end
      end
    end
  end

  describe "convert_unity_to_usd/2" do
    test "converts Unity to USD (mock when pxr unavailable)" do
      case AriaUsd.convert_unity_to_usd("/path/to/unity_asset", "/path/to/output.usd") do
        {:ok, status} ->
          assert is_binary(status)

        {:error, _reason} ->
          # Skip test if Unity conversion not available
          :ok
      end
    end

    @tag :requires_pythonx
    test "returns error for non-existent Unity asset file" do
      case AriaUsd.ensure_pythonx() do
        :mock ->
          :ok

        :ok ->
          tmp_usd = tmp_usd_path("unity_convert")

          try do
            assert {:error, _reason} =
                     AriaUsd.convert_unity_to_usd("nonexistent.unity", tmp_usd)
          after
            if File.exists?(tmp_usd), do: File.rm(tmp_usd)
          end
      end
  ...
  describe "import_unity_package/2" do
    test "imports Unity package (mock when unavailable)" do
      case AriaUsd.import_unity_package("/path/to/package.unitypackage", "/path/to/output") do
        {:ok, status} ->
          assert is_binary(status)

        {:error, _reason} ->
          # Skip test if Unity package support not available
          :ok
      end
    end

    @tag :requires_pythonx
    test "returns error for non-existent Unity package file" do
      case AriaUsd.ensure_pythonx() do
        :mock ->
          :ok

        :ok ->
          tmp_dir = System.tmp_dir!() |> Path.join("unity_import_#{:rand.uniform(1_000_000)}")
          File.mkdir_p!(tmp_dir)

          try do
            assert {:error, _reason} =
                     AriaUsd.import_unity_package("nonexistent.unitypackage", tmp_dir)
          after
